...
export interface PenilaianUkk {
  ...
  nilai_akhir: number; catatan: string | null; status: 'draft' | 'final'
  created_at: string; updated_at: string
  peserta?: PesertaSeleksi; tahapan?: TahapanSeleksi
}

// ── PENGUMUMAN ────────────────────────────────────────────────
export interface Pengumuman { id: string; judul: string; isi: string | null; kategori: 'umum' | 'seleksi' | 'monev' | 'regulasi' | 'sop'; is_publik: boolean; file_url: string | null; seleksi_id: string | null; created_by: string | null; created_at: string; updated_at: string }

// ── NOTIFIKASI ────────────────────────────────────────────────
export interface Notifikasi { id: string; user_id: string | null; peserta_id: string | null; judul: string; isi: string | null; kategori: 'info' | 'sukses' | 'peringatan' | 'error'; is_read: boolean; action_url: string | null; created_at: string }

// ── VIEWS ─────────────────────────────────────────────────────
export interface PesertaLengkap extends PesertaSeleksi { seleksi_judul: string; seleksi_jenis: JenisSeleksi; seleksi_entitas: 'BUMD' | 'BLUD'; total_dokumen: number; dokumen_verified: number; dokumen_ditolak: number; dokumen_pending: number }
export interface Statistik { total_bumd: number; total_blud: number; total_seleksi: number; total_peserta: number; total_regulasi: number; total_sop: number }

// ── FORM TYPES ────────────────────────────────────────────────
export interface RegisterPesertaForm { seleksi_id: string; nik: string; nama: string; ttl: string; alamat: string; pendidikan: string; whatsapp: string; username: string; password: string; confirmPassword: string }
export interface LoginForm { username: string; password: string }
export interface VerifikasiDokumenForm { status_verifikasi: StatusVerifikasi; catatan: string }
export interface InputNilaiForm { peserta_id: string; tahapan_id: string; nilai: number; status: 'lulus' | 'tidak_lulus' | 'absen'; catatan: string }
export interface GantiPasswordForm { password_lama: string; password_baru: string; konfirmasi_password: string }
...
// ── LABELS ────────────────────────────────────────────────────
export const DOKUMEN_LABELS: Record<JenisDokumen, string> = { ktp: 'Kartu Tanda Penduduk (KTP)', ijazah: 'Ijazah Terakhir', cv: 'Curriculum Vitae (CV)', skck: 'SKCK', surat_kesehatan: 'Surat Keterangan Sehat', pakta_integritas: 'Pakta Integritas', foto: 'Pas Foto', dokumen_pendukung: 'Dokumen Pendukung' }
export const STATUS_PESERTA_LABELS: Record<StatusPeserta, string> = { terdaftar: 'Terdaftar', verifikasi_dokumen: 'Verifikasi Dokumen', lulus_admin: 'Lulus Administrasi', tms_admin: 'TMS Administrasi', undangan_ukk: 'Undangan UKK', lulus_ukk: 'Lulus UKK', tms_ukk: 'TMS UKK', lulus_akhir: 'Lulus Akhir', tidak_lulus: 'Tidak Lulus' }
export const STATUS_VERIFIKASI_LABELS: Record<StatusVerifikasi, string> = { pending: 'Menunggu Verifikasi', diverifikasi: 'Terverifikasi', ditolak: 'Ditolak' }

// ── MENU ACCESS PER ROLE ──────────────────────────────────────
export const ROLE_MENU_ACCESS: Record<RoleName, string[]> = {
  super_admin:     ['dashboard','monev_bumd','monev_blud','seleksi','regulasi','sop','pengumuman','laporan','users'],
  admin_bumd:      ['dashboard','monev_bumd','laporan'],
  admin_blud:      ['dashboard','monev_blud','laporan'],
  admin_bpsda:     ['dashboard','monev_bumd','monev_blud'],
  panitia_seleksi: ['dashboard','seleksi','pengumuman'],
  tim_ukk:         ['dashboard','seleksi'],
  peserta:         ['portal_seleksi'],
  tim_seleksi:     ['dashboard','seleksi','pengumuman'],
  viewer:          ['dashboard'],
}
