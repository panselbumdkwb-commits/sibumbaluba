...
export default function TambahMonevBlud() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const persentase = form.rba.pagu && form.rba.realisasi
        ? Math.round((parseFloat(form.rba.realisasi) / parseFloat(form.rba.pagu)) * 10000) / 100
        : null
      ...
      const { error } = await supabase.from('monev_blud').insert({
        ...
        rba: {
          pagu: toNum(form.rba.pagu),
          realisasi: toNum(form.rba.realisasi),
          persentase,
        },
        laporan_keuangan: {},
        laporan_kinerja: {},
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <button onClick={() => router.push('/monev/blud')}
          ...
          <ArrowLeft className="h-4 w-4" /> Kembali ke Monev BLUD
        </button>
        ...
        <Section title="Identitas Data">
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              ...
              <BludSelect value={form.blud_id} onChange={v => setForm(f => ({ ...f, blud_id: v }))} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Periode <span className="text-red-500">*</span></label>
              <select value={form.periode} onChange={e => setForm(f => ({ ...f, periode: e.target.value }))} className={inputCls}>
          ...
          <div>
            ...
            <select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as 'draft'|'submitted' }))} className={inputCls}>
        ...
        <Section title="Rencana Bisnis Anggaran (RBA)">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={form.rba.pagu} onChange={e => setForm(f => ({ ...f, rba: { ...f.rba, pagu: e.target.value } }))}
                placeholder="0" className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Realisasi Anggaran (Rp)</label>
              <input type="number" value={form.rba.realisasi} onChange={e => setForm(f => ({ ...f, rba: { ...f.rba, realisasi: e.target.value } }))}
                placeholder="0" className={inputCls} />
            </div>
          </div>
          {form.rba.pagu && form.rba.realisasi && (
            <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 text-sm">
              ...
              <span className="font-bold text-emerald-700 dark:text-emerald-400">
                {((parseFloat(form.rba.realisasi) / parseFloat(form.rba.pagu)) * 100).toFixed(2)}%
              </span>
            </div>
          )}
        </Section>
        ...
        <Section title="Status Akreditasi">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.akreditasi.status} onChange={e => setForm(f => ({ ...f, akreditasi: { ...f.akreditasi, status: e.target.value } }))} className={inputCls}>
                <option value="">Pilih...</option>
            ...
            <div>
              ...
              <input type="text" value={form.akreditasi.tahun} onChange={e => setForm(f => ({ ...f, akreditasi: { ...f.akreditasi, tahun: e.target.value } }))}
                placeholder="cth: 2023" className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Masa Berlaku s/d</label>
              <input type="date" value={form.akreditasi.masa_berlaku} onChange={e => setForm(f => ({ ...f, akreditasi: { ...f.akreditasi, masa_berlaku: e.target.value } }))}
                className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Catatan</label>
              <input type="text" value={form.akreditasi.catatan} onChange={e => setForm(f => ({ ...f, akreditasi: { ...f.akreditasi, catatan: e.target.value } }))}
                  ...
                  <tr key={s.indikator} className="border-b border-border/50">
                    ...
                    <td className="px-3 py-2">
                      <input type="number" min="0" max="100" value={s.target}
                        onChange={e => setForm(f => ({ ...f, spm: f.spm.map((sp, idx) => idx === i ? { ...sp, target: e.target.value } : sp) }))}
                    ...
                    <td className="px-3 py-2">
                      <input type="number" min="0" max="100" value={s.capaian}
                        onChange={e => setForm(f => ({ ...f, spm: f.spm.map((sp, idx) => idx === i ? { ...sp, capaian: e.target.value } : sp) }))}
                    ...
                    <td className="px-3 py-2">
                      <input type="text" value={s.keterangan}
                        onChange={e => setForm(f => ({ ...f, spm: f.spm.map((sp, idx) => idx === i ? { ...sp, keterangan: e.target.value } : sp) }))}
...
function BludSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const [blud, setBlud] = useState<Array<{ id: string; nama: string }>>([])
  ...
  return (
    <select value={value} onChange={e => onChange(e.target.value)} onFocus={load}
      ...
      {blud.map(b => <option key={b.id} value={b.id}>{b.nama}</option>)}
    </select>
  )
}
