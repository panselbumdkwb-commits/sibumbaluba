...
const JENIS_SELEKSI = ['Direksi', 'Dewan Pengawas', 'Dewas', 'Direktur', 'Komisaris', 'lainnya']

interface Tahapan { nama_tahap: string; urutan: number; tanggal_mulai: string; tanggal_selesai: string }

export default function SeleksiBaru() {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({
    judul: '', jenis: '', entitas: '', status: 'draft',
    persyaratan: '', kuota: 1,
  ...
  function setJadwal(k: string, v: string) { setForm(f => ({ ...f, jadwal: { ...f.jadwal, [k]: v } })) }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.judul || !form.jenis || !form.entitas) {
      toast.error('Judul, jenis, dan entitas wajib diisi'); return
    }
    setLoading(true)
    try {
      ...
      const tahapanData = tahapan
        .filter(t => t.nama_tahap.trim())
        .map(t => ({ ...t, seleksi_id: seleksi.id }))

      if (tahapanData.length > 0) {
        await supabase.from('tahapan_seleksi').insert(tahapanData)
      }
  ...
  function addTahapan() {
    setTahapan(prev => [...prev, { nama_tahap: '', urutan: prev.length + 1, tanggal_mulai: '', tanggal_selesai: '' }])
  }
  function removeTahapan(i: number) {
    setTahapan(prev => prev.filter((_, idx) => idx !== i).map((t, idx) => ({ ...t, urutan: idx + 1 })))
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.judul} onChange={e => setField('judul', e.target.value)}
              placeholder="cth: Seleksi Direksi Perumdam Among Tani 2025"
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              ...
              <select value={form.entitas} onChange={e => setField('entitas', e.target.value)}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="1" value={form.kuota} onChange={e => setField('kuota', parseInt(e.target.value))}
                className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1.5">Persyaratan Umum</label>
            <textarea value={form.persyaratan} onChange={e => setField('persyaratan', e.target.value)} rows={4}
              placeholder="Tuliskan persyaratan umum seleksi..."
        ...
        {/* Jadwal */}
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <h2 className="font-bold">Jadwal Seleksi</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={k}>
                ...
                <input type="date" value={form.jadwal[k as keyof typeof form.jadwal]}
                  onChange={e => setJadwal(k, e.target.value)}
                  className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
            ))}
          </div>
            ...
            {tahapan.map((t, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border/50">
                ...
                <input value={t.nama_tahap}
                  onChange={e => setTahapan(prev => prev.map((p, idx) => idx === i ? { ...p, nama_tahap: e.target.value } : p))}
                ...
                <input type="date" value={t.tanggal_mulai}
                  onChange={e => setTahapan(prev => prev.map((p, idx) => idx === i ? { ...p, tanggal_mulai: e.target.value } : p))}
                  className="h-9 px-3 rounded-lg border border-input bg-background text-xs focus:outline-none" />
                <input type="date" value={t.tanggal_selesai}
                  onChange={e => setTahapan(prev => prev.map((p, idx) => idx === i ? { ...p, tanggal_selesai: e.target.value } : p))}
                  className="h-9 px-3 rounded-lg border border-input bg-background text-xs focus:outline-none" />
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={() => router.push('/seleksi')}
            className="px-6 py-2.5 rounded-xl border border-border hover:bg-accent text-sm font-medium transition-colors">
            Batal
          </button>
          <button type="submit" disabled={loading}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 disabled:opacity-60 transition-colors">
