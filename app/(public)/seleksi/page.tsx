...
export default async function SeleksiPublikPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      {aktif.length > 0 && (
        <div className="mb-10">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {aktif.map(s => (
              <SeleksiCard key={s.id} seleksi={s} showDaftar />
            ))}
          </div>
        </div>
      )}
      ...
      {lainnya.length > 0 && (
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {lainnya.map(s => (
              <SeleksiCard key={s.id} seleksi={s} />
            ))}
          </div>
        </div>
      )}
      ...
      {/* Form Registrasi Modal tersedia via client component */}
      <FormRegistrasiSeleksi seleksiAktif={aktif} />
    </div>
  )
}

function SeleksiCard({ seleksi: s, showDaftar = false }: { seleksi: Seleksi; showDaftar?: boolean }) {
  return (
    <div id={s.id} className="border border-border bg-card rounded-xl p-6 hover:shadow-md transition-all">
      {/* Header */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex-1">
          <h3 className="font-bold text-foreground mb-1">{s.judul}</h3>
        ...
        <div className="space-y-2 mb-4">
          {s.jadwal.pendaftaran_mulai && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Pendaftaran: {formatDate(s.jadwal.pendaftaran_mulai)} – {s.jadwal.pendaftaran_selesai ? formatDate(s.jadwal.pendaftaran_selesai) : '?'}</span>
            </div>
          )}
      ...
      {showDaftar && s.status === 'buka' && (
        <a
          href={`/seleksi/daftar?seleksi_id=${s.id}`}
