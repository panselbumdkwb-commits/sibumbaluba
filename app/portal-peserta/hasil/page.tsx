...
import { Trophy, CheckCircle2, XCircle, Clock, ArrowLeft } from 'lucide-react'
...
async function getData() {
  ...
  const { data: peserta } = await supabase
    .from('peserta_seleksi').select('*, seleksi(*)').eq('auth_user_id', user.id).single()
  if (!peserta) redirect('/portal-peserta/login')

  const { data: hasil } = await supabase
    .from('hasil_seleksi')
    .select('*, tahapan:tahapan_seleksi(nama_tahap, urutan)')
    .eq('peserta_id', peserta.id)
    .order('tahapan(urutan)')

  return { peserta, hasil: hasil ?? [] }
}
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className={`rounded-2xl p-8 mb-6 text-center ${
          ...
          <div className="flex justify-center mb-4">
            {isLulus ? <Trophy className="h-16 w-16 text-yellow-300" /> :
             isTidakLulus ? <XCircle className="h-16 w-16 text-white/80" /> :
             <Clock className="h-16 w-16 text-white/80" />}
          </div>
          <h1 className="text-2xl font-bold mb-2">{peserta.nama}</h1>
          <p className="text-white/80 mb-4">{peserta.nomor_peserta} · {(peserta.seleksi as Record<string, string>)?.judul}</p>
          <div className="inline-block px-4 py-2 rounded-full bg-white/20 font-bold text-lg">
            {STATUS_PESERTA_LABELS[peserta.status as keyof typeof STATUS_PESERTA_LABELS]}
          </div>
          {isLulus && (
        ...
        {hasil.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-6">
            ...
            <div className="space-y-4">
              {hasil.map(h => {
                const tahapan = h.tahapan as { nama_tahap: string; urutan: number } | null
                return (
                  <div key={h.id} className="flex items-center gap-4 p-4 rounded-xl border border-border">
                    ...
                    <div className="flex-1">
                      <div className="font-semibold text-sm">{tahapan?.nama_tahap ?? 'Tahapan'}</div>
